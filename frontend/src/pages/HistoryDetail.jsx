...
const HistoryDetail = () => {
  ...
  return (
    <>
      ...
      {searchResult.SearchQuery.length > 40 ? 
        <h1 className='search_term_short h1-custom'>{searchResult.SearchQuery.slice(0, 40)} ...  <span className='search_term_full'>{searchResult.SearchQuery}</span> </h1> 
      :<h1 className='h1-custom'>{searchResult.SearchQuery}</h1> }

      <div className={"content"}>
        <>
          <div ><img src={searchResult.ImageUrl} alt={searchResult.SearchQuery} /><Feedback setLike={setImageFeedback}/></div>
